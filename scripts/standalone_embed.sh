...
##  change COMMON_STORAGETYPE local->remote 
...
#    res=`sudo docker ps -a|grep milvus-standalone|wc -l` ## the `ps -a` start the image with the last configuration
    res=`sudo docker ps |grep milvus-standalone|wc -l` ## this line will `docker run` the Milvus 
    if [ $res -eq 1 ]
    then
